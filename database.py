...
async def reply_with_mention(message: Message, text: str):
    ...
    nickname = db.get_name(peer_id, user_id)
    if nickname:
...
async def handle_message(message: Message):
    ...
    db.update_activity(peer_id, user_id, is_roll=False)
    ...
    if lower in ('/имена', '/кимена'):
        all_names = db.get_all_names(peer_id)
        if not all_names:
    ...
    if lower == '/вышедшие кик' or lower == '/к вышедшие кик':
        ...
        if not members:
            ...
            return
        left = db.remove_left_users(peer_id, members)
        if left:
    ...
    if lower.startswith('/имя') or lower.startswith('/кимя'):
        ...
        if not rest:
            name = db.get_name(peer_id, user_id)
            if name:
            ...
            if len(rest) > 32:
                ...
                db.set_name(peer_id, user_id, rest)
    ...
    for token in tokens:
        ...
        if ok:
            db.update_activity(peer_id, user_id, is_roll=True)
            if prefix:
...
# ---------------------- Запуск бота (без веб-сервера) ----------------------
if __name__ == "__main__":
    # Бот запускается синхронно, vkbottle сам создаст event loop
    print("Бот запущен...")
    bot.run_forever()
